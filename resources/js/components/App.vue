<template>
    <div class="min-h-screen transition-colors duration-300" :class="isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'">
        ...
        <div v-if="showAddProduct" class="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center">
            <div class="p-6 rounded-lg shadow w-full max-w-2xl transition-colors duration-200" :class="isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'">
                ...
                <div class="flex space-x-2">
                    ...
                    <button @click="showAddProduct = false" class="cursor-pointer bg-gray-600 text-white p-2 rounded-md hover:opacity-80 transition-opacity duration-150">Cancel</button>
                </div>
            </div>
        </div>

        <!-- Alert for Unauthenticated Cart -->
        <div v-if="showAlert" class="fixed bottom-4 right-4 bg-red-600 text-white p-4 rounded-lg shadow">
            ...
            <button @click="showAlert = false" class="cursor-pointer ml-2 underline hover:opacity-80 transition-opacity">Close</button>
        </div>
    </div>
</template>
...
export default {
    data() {
        return {
            ...
            register_form: { name: '', email: '', password: '', password_confirmation: '' },
            newKnife: {
    ...
    methods: {
        async fetchKnives(page = 1) {
            try {
                ...
                this.applySearch();
                if (this.filteredKnives.length === 0 && this.searchQuery) {
        ...
        applySearch() {
            ...
            this.filteredKnives = this.knives.filter(knife =>
                knife.name.toLowerCase().includes(query)
            );
        },
